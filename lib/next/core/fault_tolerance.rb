  ...
  class Core
    ...
    module FaultTolerance
      ...
      private def handle_recreate(cause)
        log.debug("restarting", identity.name) if system.config.next.debug.lifecycle
        ...
        if actor_initialized?
          ...
          log.debug("restarted", identity.name) if system.config.next.debug.lifecycle
        else
          # log
          create_on_failure
        end
      ...
      private def handle_terminate
        ...
        log.debug("stopping", identity.name) if system.config.next.debug.lifecycle
      ...
      private def finish_terminate
        ...
        begin
          ...
          log.debug("stopped", identity.name) if system.config.next.debug.lifecycle
